...
def makeList():
    ...
    listName = input()
    with open(os.path.join(getListsFolderLocation, f"{listName}.txt"), "w") as f:
        f.write("")
...
def selectList():
    ...
    if checkTaskListExists(selectedList):
        with open(os.path.join(getListsFolderLocation(), f"{selectedList}.txt"),'r') as f:
            print(f.read())
...
def main():
    ...
    while(not exitBool and currentList == None):
        print("1. Make List")
        print("2. Select List")
        print("3. Delete List")
